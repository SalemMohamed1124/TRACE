...
function ScanFormWrapper() {
  ...
  return (
    <form
      ...
      className="grid grid-rows-[1fr_auto] min-h-0 max-h-[82vh] sm:max-h-[75vh]"
    >
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-2 px-6 py-4 border-t border-border bg-background">
        <div className="flex gap-2 w-full sm:w-auto">
          <Button
            ...
            onClick={close}
            disabled={isLoading}
            className="flex-1 sm:flex-none h-9 px-4 text-xs"
          >
            Cancel
          </Button>
          <Button
        ...
        <Button
          ...
          disabled={isLoading}
          variant="primary"
          className="w-full sm:w-auto h-9 px-6 text-xs  gap-2"
        >
          {isLoading ? <Loader2 className="animate-spin text=" /> : <Play />}
          Start Scan
        </Button>
      </div>
    </form>
        ...
        render={({ field }) => (
          <Combobox
            ...
            filter={(asset, query) => {
              ...
              return (
                ...
                asset.value.toLowerCase().includes(searchTerm)
              );
            }}
          ...
          >
            ...
            <ComboboxContent className="bg-popover border-border rounded-lg overflow-hidden shadow-lg z-100">
              <ComboboxEmpty className="p-4 text-center text-muted-foreground text-xs">
                No assets found
              </ComboboxEmpty>
              <ComboboxList className="max-h-64 custom-scrollbar p-1">
                {(asset) => (
                  <ComboboxItem
                    key={asset.id}
                    value={asset}
                    className="px-3 py-2 data-highlighted:bg-muted data-highlighted:text-foreground transition-colors border-b border-border/5 last:border-0 rounded-md"
                  >
                    <ItemContent>
                      <div className="flex items-center gap-2.5">
                        <div className="flex flex-col">
                          ...
                          <ItemDescription className="text-[10px] text-muted-foreground font-mono">
                            {asset.value}
                          </ItemDescription>
                        </div>
                      </div>
  ...
  return (
    <div className="p-4 rounded-lg bg-muted/5 border border-border">
      <div className="flex items-center gap-2 mb-3">
        <h4 className="font-semibold text-xs text-foreground ml-0.5">
      ...
      {authMode === "credentials" && (
        <div className="space-y-2.5 animate-in fade-in slide-in-from-top-1 duration-200">
          ...
          <Input
            ...
            {...control.register("loginUrl")}
            className="h-9 text-xs bg-background border-border rounded-md font-mono"
          />
        </div>
      )}
      ...
      {authMode === "cookies" && (
        <div className="space-y-2.5 animate-in fade-in slide-in-from-top-1 duration-200">
          <Input
            placeholder="Cookies (name=value; ...)"
            {...control.register("cookies")}
            className="h-9 text-[10px] bg-background border-border rounded-md font-mono"
          />
          <Input
            placeholder="Custom Headers (JSON or Key:Value)"
            {...control.register("customHeaders")}
            className="h-9 text-[10px] bg-background border-border rounded-md font-mono"
          />
        </div>
      )}
